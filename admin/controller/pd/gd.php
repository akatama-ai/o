<?php
class ControllerPdGd extends Controller {
	public function index() {
		
		$this->document->setTitle('Withdrawal Success');
		...
		$ts_history = $this -> model_pd_registercustom -> get_count_withdrawal_success();
		$data['self'] =  $this;
		...
		$pagination -> url = $this -> url -> link('pd/gd', 'page={page}&token='.$this->session->data['token'].'', 'SSL');
		$data['code'] =  $this-> model_pd_registercustom->get_all_withdrawal_success($limit, $start);
		...
		$this->response->setOutput($this->load->view('pd/gd.tpl', $data));
	}
